...
public:
  ...
  DA2Network( const char *network_path ) {
    ...
    // convert UTF-8/ANSI path to wide string for the Ort wchar_t overload
    std::vector<wchar_t> wpath;
    int sz = MultiByteToWideChar(CP_UTF8, 0, network_path, -1, NULL, 0);
    if (sz > 0) {
      wpath.resize(sz);
      MultiByteToWideChar(CP_UTF8, 0, network_path, -1, wpath.data(), sz);
    } else {
      // fallback: attempt simple widen
      size_t len = strlen(network_path);
      wpath.resize(len + 1);
      for (size_t i = 0; i <= len; ++i) wpath[i] = (wchar_t)network_path[i];
    }
    this->session_ = new Ort::Session(env, wpath.data(), Ort::SessionOptions{nullptr});
  }
